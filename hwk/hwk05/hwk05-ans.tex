...
\begin{itemize}
    ...
    \(X_1, \ldots, X_8\) be average age of patients, infection risk, cultering ratio, X-ray ratio, number of beds, average daily census, number of nurses, and 
    available facilities and services, respectively. To find optimal parameters for transformation, we will fit the model 
    \begin{align}
        g_{\theta}(Y)
        = \beta_0 + \sum_{i = 1}^8 \beta_i \cdot g_{\lambda_{i}}(X_i) + \epsilon,
    ...
    \(\gmat = \begin{bmatrix}
        \mathbf{1} & \mathbf{g}_{\lambda_1}(\mathbf{x}_1) & \mathbf{g}_{\lambda_8}(\mathbf{x}_8)
    \end{bmatrix}\)
    is the matrix of transformed predictors. It can be shown (see Appendix B) that the log-likelihood can be expressed as a function of only the
